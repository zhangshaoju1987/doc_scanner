...
import Animated, { useAnimatedGestureHandler, useAnimatedStyle, useSharedValue, withDecay, withTiming, Easing, } from 'react-native-reanimated';
...
function Example() {
    const headerHeight = useHeaderHeight();
    const height = Platform.OS === 'web' ? windowDimensions.height - headerHeight : undefined;
    return (<View style={[styles.wrapper, { height }]}>
      <ScrollableView>
        {colors.map((color) => (<Box color={color} key={color}/>))}
      </ScrollableView>
    </View>);
}
