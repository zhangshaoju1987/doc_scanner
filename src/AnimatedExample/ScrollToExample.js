...
import Animated, { useSharedValue, useAnimatedStyle, useAnimatedGestureHandler, scrollTo, useDerivedValue, useAnimatedRef, } from 'react-native-reanimated';
...
function getDigit(number, i) {
    return useDerivedValue(() => {
        return Math.floor(number.value / Math.pow(10, i)) % 10;
    });
}
function NumberDisplay({ number }) {
    return (<View style={{ height: 400, width: 200 }}>
      <View style={{
...
function Digit({ digit }) {
    ...
    return (<View style={{ height: 200, width: Platform.OS === 'web' ? 50 : undefined }}>
...
function ProgressBar({ progress }) {
    ...
    return (<View style={{ height: 100, paddingRight: 80, paddingLeft: 40, width: 300 }}>
      <View onLayout={(e) => {
