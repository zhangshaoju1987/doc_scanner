...
import Animated, { useSharedValue, useAnimatedStyle, useAnimatedScrollHandler, useAnimatedRef, scrollTo, useAnimatedGestureHandler, Extrapolate, interpolate, } from 'react-native-reanimated';
...
const SCREEN_WIDTH = Dimensions.get('window').width - IPOD_MARGIN * 2 - SCROLL_MARGIN * 2;
...
function ScrollExample() {
    ...
    const gestureHandler = useAnimatedGestureHandler({
        ...
        onActive: (e, ctx) => {
            ...
            position.value = interpolate(position.value + sign * dist * 5, arr, arr, Extrapolate.CLAMP);
    ...
    return (<View style={styles.ipod}>
      <Animated.ScrollView ref={animatedRef} horizontal={true} style={styles.scroll} scrollEventThrottle={1} showsHorizontalScrollIndicator={false} onScroll={scrollHandler}>
        {data.map(({ artist, song }, i) => {
        const uas = useAnimatedStyle(() => {
            const style = {};
            const itemDistance = Math.abs(position.value - i * itemTotalSize) / itemTotalSize;
            let opacity = 1;
            if (itemDistance >= 0.5) {
                opacity = 0.3;
            }
        ...
        return (<Animated.View key={i} style={[styles.item, uas]}>
