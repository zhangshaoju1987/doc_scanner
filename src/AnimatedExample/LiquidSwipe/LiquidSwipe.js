...
import Animated, { useSharedValue, useAnimatedGestureHandler, cancelAnimation, interpolate, Extrapolate, withSpring, } from 'react-native-reanimated';
...
function LiquidSwipe() {
    ...
    const handler = useAnimatedGestureHandler({
        ...
        onActive: (event, ctx) => {
            ...
            if (isBack.value) {
                ...
                progress.value = interpolate(event.translationX, [-maxDist, 0], [0.4, 0], Extrapolate.CLAMP);
            }
        },
        onEnd: () => {
            let goBack;
    ...
    return (<View style={styles.container}>
      <Content backgroundColor="white" source={assets[0]} title1="Online" title2="Gambling" color="black"/>
      <PanGestureHandler onGestureEvent={handler} onHandlerStateChange={handler}>
        <Animated.View style={StyleSheet.absoluteFill}>
          <Weave progress={progress} centerY={centerY} isBack={isBack}>
            <Content backgroundColor="#4d1168" source={assets[1]} title1="For" title2="Gamers" color="#fd5587"/>
          </Weave>
