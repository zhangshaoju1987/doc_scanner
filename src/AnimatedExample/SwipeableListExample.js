...
import Animated, { useSharedValue, useAnimatedGestureHandler, useAnimatedStyle, withSpring, withTiming, Easing, runOnJS, } from 'react-native-reanimated';
...
function SwipableList() {
    ...
    return (<View style={s.container}>
      <FlatList data={data} renderItem={({ item }) => <ListItem item={item} onRemove={onRemove}/>} keyExtractor={(item) => item.id}/>
    </View>);
}
...
function Button({ item }) {
    return (<View style={[s.button, { backgroundColor: item.backgroundColor }]}>
...
function ListItemContent({ item }) {
    return (<View style={s.itemContainer}>
