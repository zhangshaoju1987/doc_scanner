...
import Animated, { useSharedValue, useAnimatedStyle, useAnimatedGestureHandler, interpolate, Extrapolate, withTiming, Easing, useAnimatedRef, measure, runOnJS, } from 'react-native-reanimated';
import { Dimensions, StyleSheet, View, Image, Platform, StatusBar, } from 'react-native';
import { ScrollView, PanGestureHandler, TapGestureHandler, } from 'react-native-gesture-handler';
...
function ImageList({ images, onItemPress, }) {
    return (<ScrollView contentContainerStyle={styles.scrollContainer}>
      {images.map((item, i) => (<ListItem onPress={onItemPress} key={i} index={i} item={item}/>))}
    </ScrollView>);
}
function ListItem({ item, index, onPress }) {
    ...
    return (<TapGestureHandler onGestureEvent={handler}>
...
function ImageTransition({ activeImage, onClose, }) {
    ...
    const onPan = useAnimatedGestureHandler({
        onActive: (event) => {
            ...
            scale.value = interpolate(translateY.value, [-200, 0, 200], [0.65, 1, 0.65], Extrapolate.CLAMP);
            backdropOpacity.value = interpolate(translateY.value, [-100, 0, 100], [0, 1, 0], Extrapolate.CLAMP);
        },
        onEnd: (_event, _ctx) => {
    ...
    const imageStyles = useAnimatedStyle(() => {
        const interpolateProgress = (range) => interpolate(animationProgress.value, [0, 1], range, Extrapolate.CLAMP);
...
function LightboxExample() {
    ...
    animatedRef, item, svs) {
        setActiveImage(Object.assign({ animatedRef,
            item }, svs));
    }
    function onClose() {
        setActiveImage(null);
    }
    const headerHeight = useHeaderHeight() - ((_a = StatusBar.currentHeight) !== null && _a !== void 0 ? _a : 0);
...
const styles = StyleSheet.create({
    ...
    backdrop: Object.assign(Object.assign({}, StyleSheet.absoluteFillObject), { backgroundColor: 'black' }),
});
