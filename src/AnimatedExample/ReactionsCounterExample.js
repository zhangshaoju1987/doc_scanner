...
function ReactionsCounter({ emoji, count, you, onPress, }) {
    const oldCount = useSharedValue(-1);
    const newCount = useSharedValue(count);
    React.useEffect(() => {
        newCount.value = count;
    }, [count]);
    const animatedStyle = useAnimatedStyle(() => {
        return {
            backgroundColor: withTiming(you ? BACKGROUND_COLOR_ON : BACKGROUND_COLOR_OFF, { duration: COLOR_ANIMATION_DURATION }),
    ...
    const entering = (values) => {
        ...
        oldCount.value = count;
        const animations = {
            originY: withDelay(COUNT_ANIMATION_DELAY, withTiming(values.targetOriginY, { duration: COUNT_ANIMATION_DURATION })),
        };
        const initialValues = {
    ...
    const exiting = (values) => {
        'worklet';
        var _a;
        const offset = (count > ((_a = newCount.value) !== null && _a !== void 0 ? _a : count) ? 1 : -1) * COUNT_ANIMATION_DISTANCE;
        const animations = {
            originY: withDelay(COUNT_ANIMATION_DELAY, withTiming(values.currentOriginY + offset, {
    ...
    return (<TouchableWithoutFeedback onPress={onPress}>
      <Animated.View style={[styles.counter, animatedStyle]}>
        <View style={styles.contents}>
          ...
          <Animated.Text key={count} style={styles.count} entering={entering} exiting={exiting}>
            {count}
          </Animated.Text>
        </View>
      </Animated.View>
...
export function ReactionsCounterExample() {
    ...
    return (<View style={styles.container}>
      <ReactionsCounter emoji="🚀" count={count} you={you} onPress={toggleYou}/>
