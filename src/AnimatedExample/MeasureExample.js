...
import Animated, { useSharedValue, useAnimatedStyle, useAnimatedGestureHandler, measure, withTiming, Easing, useDerivedValue, useAnimatedRef, } from 'react-native-reanimated';
...
function createSharedVariables() {
    ...
    for (let i = 1; i < indices.length; i++) {
        ...
        result.push(useDerivedValue(() => {
            return (previousHeight.value +
...
function MeasureExample() {
    ...
    return (<View>
      <SafeAreaView>
        <View>
          {indices.map((i) => {
        return (<Section title={days[i]} key={i} height={heights[i]} contentHeight={contentHeights[i]} z={i} show={true}>
...
function Section({ title, children, height, contentHeight, z, show, }) {
    ...
    return (<Animated.View style={[styles.section, stylez, { zIndex: z }]}>
      <SectionHeader title={title} animatedRef={aref} contentHeight={contentHeight} show={show}/>
      <View>
        {React.Children.map(children, (element) => React.cloneElement(element, { ref: aref }))}
      </View>
    </Animated.View>);
}
function asyncMeasure(animatedRef) {
    return new Promise((resolve, reject) => {
        ...
        if (animatedRef && animatedRef.current) {
            (_b = (_a = animatedRef.current).measure) === null || _b === void 0 ? void 0 : _b.call(_a, (x, y, width, height, pageX, pageY) => {
...
function SectionHeader({ title, animatedRef, contentHeight, show, }) {
    ...
    let onActiveImpl;
    if (Platform.OS === 'web') {
        onActiveImpl = () => __awaiter(this, void 0, void 0, function* () {
            try {
                applyMeasure(yield asyncMeasure(animatedRef));
            }
            catch (e) {
                console.log(e);
    ...
    return (<View style={styles.sectionHeader}>
      <View style={{
        ...
        <Text>{title}</Text>
        {show && (<TapGestureHandler onHandlerStateChange={handler}>
...
function RandomElement() {
    ...
    return (<View style={[styles.randomElement, { height: randomHeight.current }]}>
