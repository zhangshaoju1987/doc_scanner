...
function PaginationElement({ position, slideIndex, }) {
    ...
    const dotAnimatedStyle = useAnimatedStyle(() => {
        const width = interpolate(position.value, inputRange, [4, 40, 4], Extrapolate.CLAMP);
        return { width };
    });
    const dotContainerAnimatedStyle = useAnimatedStyle(() => {
        const opacity = interpolate(position.value, inputRange, [0.4, 1, 0.4], Extrapolate.CLAMP);
        return { opacity };
    });
    return (<Animated.View style={[styles.paginationElementContainer, dotContainerAnimatedStyle]}>
